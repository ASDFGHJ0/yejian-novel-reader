...
type Note = { id: string; chapter: number; quote: string; text: string; createdAt: number };
type BookMeta = { id: string; title: string; chapterTitles: string[]; at: number; createdAt?: number; current: number; size: number; scroll?: number; bookmarks?: number[]; notes?: Note[]; readSeconds?: number };
type BookRecord = BookMeta & { chapters: Chapter[] };
type InstallPromptEvent = Event & { prompt: () => Promise<void>; userChoice: Promise<{ outcome: "accepted" | "dismissed" }> };
type ReaderSettings = { font: number; lineHeight: number; width: number; family: string };
type SearchHit = { book: BookMeta; chapterIndex: number; chapterTitle: string; excerpt: string };
type BackupFile = { format: "yejian-library"; version: 1; exportedAt: string; books: BookRecord[] };
type TxtEncoding = "auto" | "utf-8" | "gb18030" | "big5";
type ExportFileHandle = { createWritable: () => Promise<{ write: (data: string) => Promise<void>; close: () => Promise<void> }> };
type ExportDirectoryHandle = { getDirectoryHandle: (name: string, options?: { create?: boolean }) => Promise<ExportDirectoryHandle>; getFileHandle: (name: string, options?: { create?: boolean }) => Promise<ExportFileHandle> };
type ExportMode = "separate" | "merged" | "grouped";
type ExportJob = { id: string; title: string; detail: string; current: number; total: number; status: "running" | "done" | "error"; error?: string };

const DEFAULT_SETTINGS: ReaderSettings = { font: 20, lineHeight: 2, width: 700, family: "serif" };

const DEMO: BookRecord = {
...
async function saveBook(record: BookRecord) {
...
async function getBooks(): Promise<BookMeta[]> {
  const db = await openLibrary();
...
async function getChapter(bookId: string, index: number): Promise<Chapter> {
  const db = await openLibrary();
...
async function updateProgress(book: BookMeta) {
  const db = await openLibrary();
...
async function deleteBook(book: BookMeta) {
  const db = await openLibrary();
  const tx = db.transaction(["books", "chapters"], "readwrite");
...
async function exportLibrary(): Promise<BackupFile> {
  const books = await getBooks();
  const records = await Promise.all(books.map(async book => ({
    ...book,
    chapters: await Promise.all(book.chapterTitles.map((_, index) => getChapter(book.id, index))),
  })));
  return { format: "yejian-library", version: 1, exportedAt: new Date().toISOString(), books: records };
}

async function restoreLibrary(backup: BackupFile) {
...
function parseNovel(text: string, name: string, bytes: number): BookRecord {
...
function readPercent(book: BookMeta) {
...
function renderMarkedText(text: string, notes: Note[]) {
...
function decodeTxt(buffer: ArrayBuffer, encoding: TxtEncoding) {
...
function chapterFileName(title: string, index: number) {
...
export default function Home() {
  ...
  if (active && chapter) return <main className={`reader ${theme}`} onClick={() => selectionMenu && setSelectionMenu(null)}>
    <header>
      <button onClick={() => { localStorage.removeItem("yejian-active-book"); setActive(null); setChapter(null); window.scrollTo({ top: 0 }); }}>←</button>
      <div><b>{active.title}</b><small>{chapter.title} · 全书 {readPercent(active)}%</small></div>
      <button className="tocTrigger" onClick={() => setToc(true)} aria-label="打开章节目录"><span>☰</span> 目录</button>
      <button className={(active.bookmarks || []).includes(active.current || 0) ? "marked" : ""} onClick={toggleBookmark} title="为当前章节添加书签">{(active.bookmarks || []).includes(active.current || 0) ? "★" : "☆"}</button>
      <button onClick={() => setNotesOpen(true)} title="查看书签与笔记">笔记</button>
      <button onClick={() => setSettingsOpen(true)}>Aa</button>
    </header>
    <div className="readingProgress"><i style={{ width: `${readPercent(active)}%` }} /></div>
    <article onContextMenu={openSelectionMenu} style={{ fontSize: settings.font, lineHeight: settings.lineHeight, maxWidth: settings.width, fontFamily: settings.family === "serif" ? "var(--serif)" : settings.family === "kai" ? "KaiTi, STKaiti, serif" : "Arial, Microsoft YaHei, sans-serif" }}>
      <i>{String(active.current + 1).padStart(2, "0")}</i><h1>{chapter.title}</h1>
      {chapter.content.split(/\n+/).filter(Boolean).map((p, i) => <p key={i}>{renderMarkedText(p, (active.notes || []).filter(note => note.chapter === (active.current || 0)))}</p>)}
      <footer>
  ...
  return <main>
    <nav><a><span>页</span><b>页间</b></a><div>我的书架　　使用帮助</div><button className="navText" onClick={() => setSearchOpen(true)}>⌕ 全书搜索</button><button className="installButton" onClick={installApp}>{installed ? "✓ 已安装" : "▣ 安装到桌面"}</button><button onClick={() => input.current?.click()}>＋ 导入小说</button></nav>
